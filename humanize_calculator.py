...
def humanize(str_input):
    ...
    for j in range(len(str_input)):
        ...
        if not re.match(exp, str_input[j]):
            number += str_input[j]
        if (j != len(str_input) - 1) and (re.match(exp, str_input[j + 1])) and (not re.match(exp, str_input[j])):
            list_input.append(number)
            number = ""
        if (not re.match(exp, str_input[j])) and (j == len(str_input) - 1):
            list_input.append(number)
